...
export const QuranClassJuzDetailView = () => {
  ...
  const { classroomId, juzId } = useParams<{ classroomId: string; juzId: string }>();
  ...
  const allClasses = useMemo(() => [...(teacherClasses || []), ...(studentClasses || [])], [
    teacherClasses,
    studentClasses,
  ]);
  const classroom = useMemo(() => allClasses.find((c) => c.id === classroomId), [
    allClasses,
    classroomId,
  ]);
          ...
          {loading ? (
            ...
            juzData.items.map((item) => (
              <HafalanCard
                ...
                item={item}
                onClick={() => {
                  // We can direct them to review/detail if required, or keep it read-only
                  // Let's redirect to standard item detail just like Al-Quran page
                  navigate(`/dashboard/pribadi/book/quran/item/${item.item_id}`);
                }}
              />
            ))
          ) : (
